# Self-protection: restore persistence if tampered, detect and kill processes attempting to terminate GEDR.
# Converted from GEDR C# job - FULL IMPLEMENTATION

param([hashtable]$ModuleConfig)

$ModuleName = "EdrSelfProtection"
...
function Write-Detection {
    param(
        [string]$Message,
        [string]$Level = "THREAT",
        [string]$LogFile = "edrselfprotection_detections.log"
    )
...
# Main execution
function Invoke-EdrSelfProtection {
    $now = Get-Date
