# Detects PowerShell tampering with PnP devices (camera, mic). Flags Enable-PnpDevice/Disable-PnpDevice when parent is NOT GEDRâ€”malware trying to enable webcam.
# Converted from GEDR C# job - FULL IMPLEMENTATION

param([hashtable]$ModuleConfig)

$ModuleName = "PnPDeviceTamperingDetection"
$script:LastRun = [DateTime]::MinValue
$script:TickInterval = 30
$script:SelfPid = $PID

$script:PnPTamperPatterns = @(
        "Enable-PnpDevice",
        "Disable-PnpDevice"
    )
...
function Write-Detection {
    param(
        [string]$Message,
        [string]$Level = "THREAT",
        [string]$LogFile = "pnpdevicetamperingdetection_detections.log"
    )
...
# Main execution
function Invoke-PnPDeviceTamperingDetection {
    $now = Get-Date
...
# Execute
Invoke-PnPDeviceTamperingDetection
